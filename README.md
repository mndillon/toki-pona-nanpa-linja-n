...
- A useful convenience is that exact hundreds can be split into separate words: 200 → Netun Inin
...
The same explicit-marker approach can be extended to structured time and date values. `Tenpo` identifies a value intended to be interpreted as a time, while `Suno` identifies a value intended to be interpreted as a date. The encoded numeric components continue to use the ordinary nanpa-linja-n digit syllables, but the opening word supplies semantic context before those components are decoded. In sitelen pona, `tenpo` and `suno` likewise serve as the opening and closing cartouche markers, with a colon immediately after the opening glyph. Unlike `Nanpa`, which identifies an unrestricted decimal number, these markers also indicate that the decoded components should be validated according to the appropriate time or date syntax. This allows otherwise similar numeric sequences to be distinguished explicitly as general numbers, times or dates while retaining the same underlying numeric encoding system.
