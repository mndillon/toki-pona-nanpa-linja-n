...
const SitelenRenderer = (() => {
  ...
  async function astToLineElements(ast, config = {}) {
    ...
    for (const line of ast.lines || []) {
      const elements = [];
      for (const seg of line.children || []) {
        if (seg.kind === 'text') {
          if (parser.mode === 'sitelen-seli-kiwen') parseSskTextSegmentToElements(seg.value, elements, { fontPx, parser });
          else __bridgeParseTextSegmentToElements(seg.value, elements, { fontPx });
        }
        else if (seg.kind === 'bracket') {
          if (parser.mode === 'sitelen-seli-kiwen') parseSskBracketSegmentToElements(seg.value, elements, { fontPx, parser });
          else __bridgeParseBracketSegmentToElements(seg.value, elements, { fontPx });
        }
        else if (seg.kind === 'quote') __bridgeParseQuoteSegmentToElements(seg.value, elements, { fontPx });
          ...
          __bridgeMakeRunElementFromCodepoints(elements, seg.cps, { fontPx, fontFamily: seg.fontFamily || FONT_FAMILY_TEXT });
        }
      }
  ...
  function emitSskExtendedGlyph(matchText, leftText, headWord, rightText, elements, fontPx) {
    ...
    if (elements.length > 0) __bridgePushGapIfNeeded(elements, __bridgeWordGapForPx(fontPx));
    __bridgeMakeRunElementFromCodepoints(elements, outCps, { fontPx, fontFamily: FONT_FAMILY_TEXT });
    return true;
  }

  function emitSskCompound(matchText, leftWord, operator, rightWord, elements, fontPx) {
    ...
    if (elements.length > 0) __bridgePushGapIfNeeded(elements, __bridgeWordGapForPx(fontPx));
    __bridgeMakeRunElementFromCodepoints(elements, [leftCp, joinCp, rightCp], { fontPx, fontFamily: FONT_FAMILY_TEXT });
    return true;
  }

  function parseSskTextSegmentToElements(segmentText, elements, { fontPx, parser = {} }) {
    ...
    while ((m = tokenRe.exec(s)) !== null) {
      ...
      if (start > pos) __bridgeParseTextSegmentToElements(s.slice(pos, start), elements, { fontPx });
      let ok = false;
      if (m[2] && (m[1] != null || m[3] != null)) {
        ok = emitSskExtendedGlyph(m[0], m[1], m[2], m[3], elements, fontPx);
      } else if (m[4] && m[5] && m[6]) {
        ok = emitSskCompound(m[0], m[4], m[5], m[6], elements, fontPx);
      } else if (m[7] && m[8]) {
        ok = emitSskExtendedGlyph(m[0], m[7], m[8], null, elements, fontPx);
      }
      if (!ok) __bridgeParseTextSegmentToElements(m[0], elements, { fontPx });
      pos = end;
    }
    if (pos < s.length) __bridgeParseTextSegmentToElements(s.slice(pos), elements, { fontPx });
  }

  function parseSskBracketSegmentToElements(bracketContent, elements, { fontPx, parser = {} }) {
    ...
    if (ok) {
      if (elements.length > 0) __bridgePushGapIfNeeded(elements, __bridgeWordGapForPx(fontPx));
      __bridgeMakeRunElementFromCodepoints(elements, [0xF1990, ...cps, 0xF1991], { fontPx, fontFamily: FONT_FAMILY_CARTOUCHE });
      return;
    }
    __bridgeParseBracketSegmentToElements(content, elements, { fontPx });
  }
    ...
    for (const L of measuredLines) {
      ...
      for (const el of L.measuredEls) {
        ...
        outLine.runs.push({
          ...
          sourceText: el.type === 'text' ? String(el.text || '') : null,
          encodedText,
  ...
  "ni>": 0xF198B,
    ...
    function parseTextSegmentToElements(segmentText, elements, { fontPx }) {
      ...
      for (const h of hits) {
        ...
        if (h.kind === "tpPhrase") {
          ...
          if (cps && cps.length) {
            makeCartoucheElementFromCodepoints(elements, cps, { fontPx, fontFamily: FONT_FAMILY_NUMBER, fgCss });
          } else {
    ...
    function parseBracketSegmentToElements(bracketContent, elements, { fontPx }) {
      ...
      try {
        ...
        if (dateCaps != null) {
          ...
          if (cpsDate && cpsDate.length) {
            makeCartoucheElementFromCodepoints(elements, cpsDate, { fontPx, fontFamily: FONT_FAMILY_NUMBER, fgCss });
            return;
          }
        }
        ...
        if (timeCaps != null) {
          ...
          if (cpsTime && cpsTime.length) {
            makeCartoucheElementFromCodepoints(elements, cpsTime, { fontPx, fontFamily: FONT_FAMILY_NUMBER, fgCss });
            return;
          }
        }
      ...
      if (parsedNumber) {
        ...
        if (cps) makeCartoucheElementFromCodepoints(elements, cps, { fontPx, fontFamily: FONT_FAMILY_NUMBER, fgCss });
        return;
      }
      ...
      if (idCps && idCps.length) {
        makeCartoucheElementFromCodepoints(elements, idCps, { fontPx, fontFamily: FONT_FAMILY_NUMBER, fgCss });
        return;
      }
    ...
    function buildCartoucheCanvasForPdf(cartEl, { fontPx }){
      ...
      // Create a fresh canvas so we can force halo off and supersample
      const c = document.createElement("canvas");

      const basePx = Math.max(8, Number(fontPx ?? cartEl?.px ?? getFontPx() ?? 56));
      const px = Math.round(basePx * scale);

      // Keep the same *relative* padding, but scaled
      ...
      const r = renderFontCartoucheToCanvas(
        ...
        Array.from(cartEl?.cps ?? []),
        {
          ...
          fontFamily: fam,
          fgCss,
          haloEnabled: false, // force NO halo in PDF cartouche images
          haloCss: "#FFFFFF",
        }
      );

      // IMPORTANT: return "logical" (unscaled) metrics for layout/placement,
      // but keep the high-res canvas for embedding.
      return {
        canvas: c,
        // physical size to place in PDF (same as on-screen cartouche size)
        w: Math.round((r.w | 0) / scale),
        h: Math.round((r.h | 0) / scale),
        baselineY: Math.round((r.baselineY | 0) / scale),

        // keep scale for draw branch if you want it
        _scale: scale,
        _wPx: r.w | 0,
        _hPx: r.h | 0,
