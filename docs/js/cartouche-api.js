 ...
 *
 * Usage:
 *   import { CartoucheApi } from './cartouche-api.js';
 *
...
export function validateTpWord(w) {
  ...
  while (i < lower.length) {
    ...
    if (i >= lower.length || !VOWELS.has(lower[i])) {
      // Human-friendly position: 1-based, show the problematic character
      const pos = i + 1;
      const ch = lower[i] ?? '(end of word)';
      return { ok: false, reason: `"${w}" has an invalid syllable at letter ${pos} ("${ch}") — toki pona words follow the pattern consonant+vowel(+n), e.g. "ma", "jan", "toki"` };
    }
    const vow = lower[i]; i++;
...
// ── Boundary validation ────────────────────────────────────────────────────
// Only checks boundaries between segments — never within a nanpa run.
export function validateBoundaries(words) {
  const segs = segmentWords(words);
  for (let si = 0; si < segs.length - 1; si++) {
    const segA = segs[si];
    const segB = segs[si + 1];
    const lastWord  = segA.words[segA.words.length - 1].toLowerCase();
    const firstWord = segB.words[0].toLowerCase();
    if (lastWord[lastWord.length - 1] !== 'n') continue;
    const nextFirst = firstWord[0];
    if (VOWELS.has(nextFirst)) continue;
    if (segA.type === 'nanpa' || segB.type === 'nanpa') continue;
    return {
      ok: false,
      reason: `"${segA.words[segA.words.length-1]}" ends with 'n' and "${segB.words[0]}" starts with consonant '${nextFirst}' — only allowed before a nanpa-linja-n number. Drop the final n or reorder.`
    };
  }
  return { ok: true };
}
...
export function segmentLetters(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    const dropN = !isLast && w.toLowerCase().endsWith('n') &&
                  nextFirst && CONSONANTS.has(nextFirst) && nextFirst !== 'n';
...
function migrateRecord(e) {
  ...
  const mode = ['random','preferred','ignore'].includes(e.mode) ? e.mode : 'random';
  ...
  return { key, words: wordList, merge: e.merge !== false, mode, cartoucheMap, forceNormal: !!e.forceNormal };
}
...
export class CartoucheApi {
  ...
  async resolvePageMap() {
    ...
    for (const entry of entries) {
      ...
      map.set(entry.key, {
        input:            rendererInput,       // always used (nanpa renders natively)
        inputForceNormal: inputForceNormal,    // used only when @db present
      ...
      for (let wlen = runWords.length; wlen >= 1; wlen--) {
        ...
        if (pageMap.has(key)) {
          ...
          if (mapVal === null) {
            ...
            // mapVal is { input, inputForceNormal, forceNormal }
            const { input, inputForceNormal, forceNormal } = mapVal;
