...
const SitelenRenderer = (() => {
  ...
  function parseSskBracketSegmentToElements(bracketContent, elements, { fontPx, parser = {}, mixedStyle = 'long', sourceBaseStart = 0, sourceKind = 'bracket', sourceSegmentIndex = null }) {
    ...
    __bridgeParseBracketSegmentToElements(content, elements, { fontPx, sourceBaseStart, sourceKind, sourceSegmentIndex, mixedStyle });
  }
...
function wireHaloControls() {
  ...
  "ni>": 0xF198B,
    ...
    function lineToElements(line, { fontPx, mixedStyle = "short" }) {
      let s = String(line ?? "");
      ...
      for (let si = 0; si < segs.length; si++) {
        ...
        const sourceSegmentIndex = si;
        if (seg.kind === "text") {
          parseTextSegmentToElements(seg.value, elements, { fontPx, sourceBaseStart: 0, sourceKind, sourceSegmentIndex, mixedStyle });
        } else if (seg.kind === "bracket") {
          parseBracketSegmentToElements(seg.value, elements, { fontPx, sourceBaseStart: 0, sourceKind, sourceSegmentIndex, mixedStyle });
